...
export default function EarningsPage() {
  ...
  useEffect(() => {
    ...
    clipperApi.earnings().then(setData).catch(() => setData(null)).finally(() => setLoading(false));
  }, [router]);
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
              ...
              <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
                ...
                <div className="text-3xl font-bold text-white mt-2">${data.totalPayouts.toFixed(2)}</div>
              </div>
              <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
                <div className="text-sm text-slate-400">Balance pendiente</div>
                <div className="text-3xl font-bold text-purple-400 mt-2">${data.pendingBalance.toFixed(2)}</div>
              </div>
            </div>
              ...
              {(['clips', 'campaigns', 'payouts'] as const).map((v) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                    view === v ? 'bg-purple-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'
                  }`}
                >
            ...
            {view === 'clips' && (
              <div className="space-y-3">
                {data.byClip.length === 0 ? (
                  <div className="text-center py-12 text-slate-400 text-sm">Sin ganancias por clips todavía</div>
                ) : (
                  data.byClip.map((item) => {
                    const breakdown = item.platformBreakdown || [];
                    return (
                      <div key={item.clipId} className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
                        ...
                        {breakdown.length > 0 && (
                          <div className="mt-3 pt-3 border-t border-slate-700/50">
                            ...
                            <div className="flex flex-wrap gap-x-4 gap-y-1">
                              {breakdown.map((pb) => (
                                <div key={pb.platform} className="flex items-center gap-1.5 text-xs text-slate-400">
            ...
            {view === 'campaigns' && (
              <div className="space-y-3">
                {data.byCampaign.length === 0 ? (
                  <div className="text-center py-12 text-slate-400 text-sm">Sin ganancias por campañas todavía</div>
                ) : (
                  data.byCampaign.map((item) => (
                    <div key={item.campaignId} className="bg-slate-800/50 border border-slate-700 rounded-xl p-4 flex items-center justify-between">
                      <div>
              ...
              <div className="space-y-3">
                {data.payouts.length === 0 ? (
                  ...
                  data.payouts.map((payout) => (
                    <div key={payout.id} className="bg-slate-800/50 border border-slate-700 rounded-xl p-4 flex items-center justify-between">
                      <div>
                      ...
                      <span className={`text-sm font-medium ${PAYOUT_STATUS_COLORS[payout.status] || 'text-slate-400'}`}>

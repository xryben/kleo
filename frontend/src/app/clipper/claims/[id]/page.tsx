...
const PLATFORM_CONFIG: Record<Platform, { label: string; icon: string; placeholder: string; regex: RegExp }> = {
  TIKTOK: {
    label: 'TikTok',
    icon: '🎵',
...
export default function ClaimDetailPage() {
  ...
  const [urls, setUrls] = useState<Record<Platform, string>>({ TIKTOK: '', INSTAGRAM: '', YOUTUBE: '' });
  const [validationErrors, setValidationErrors] = useState<Record<Platform, string>>({} as Record<Platform, string>);
    ...
    claimsApi.get(claimId)
      .then((data) => {
        ...
        if (data.submissions) {
          ...
          data.submissions.forEach((s: PlatformSubmission) => { filled[s.platform] = s.url; });
          setUrls(filled);
        }
      })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    filledUrls.forEach((p) => {
      ...
      if (err) { errors[p] = err; hasError = true; }
    });
    if (hasError) {
      ...
      <div className="min-h-screen">
        <header className="border-b border-slate-800 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <Link href="/clipper/claims" className="text-lg font-bold text-white">Cleo</Link>
          </div>
        </header>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/clipper/claims" className="text-purple-400 hover:text-purple-300 text-sm mt-4 inline-block">Volver a mis claims</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen">
      <header className="border-b border-slate-800 px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <Link href="/clipper/claims" className="text-sm text-slate-400 hover:text-white transition-colors">Mis Claims</Link>
          <span className="text-slate-600 mx-2">/</span>
          <span className="text-sm text-slate-300 font-medium truncate max-w-[200px]">{claim.clipTitle}</span>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          <span className={`text-sm font-medium px-3 py-1.5 rounded-full ${STATUS_COLORS[claim.status] || 'bg-slate-700 text-slate-400'}`}>
          ...
          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6 mb-6">
            ...
            <div className="space-y-3">
              {submissions.map((sub) => {
                ...
                return (
                  <div key={sub.platform} className="flex items-center justify-between bg-slate-900/50 rounded-lg p-4">
                    <div className="flex items-center gap-3 min-w-0">
                      <span className="text-xl flex-shrink-0">{cfg.icon}</span>
                      <div className="min-w-0">
                        <div className="text-sm font-medium text-white">{cfg.label}</div>
                        <a href={sub.url} target="_blank" rel="noopener noreferrer" className="text-xs text-purple-400 hover:text-purple-300 truncate block max-w-[250px]">
                          {sub.url}
                        </a>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 flex-shrink-0">
                      <div className="text-right">
                        <div className="text-sm font-bold text-white">{(sub.views || 0).toLocaleString()}</div>
            ...
            <div className="mt-4">
              ...
              <div className="flex h-3 rounded-full overflow-hidden bg-slate-700">
                {submissions.map((sub) => {
                  ...
                  const colors: Record<string, string> = { TIKTOK: 'bg-pink-500', INSTAGRAM: 'bg-orange-500', YOUTUBE: 'bg-red-500' };
                  return pct > 0 ? (
                    <div key={sub.platform} className={`${colors[sub.platform] || 'bg-slate-500'} transition-all`} style={{ width: `${pct}%` }} title={`${PLATFORM_CONFIG[sub.platform].label}: ${sub.views.toLocaleString()}`} />
                  ) : null;
                })}
              </div>
              <div className="flex gap-4 mt-2">
                {submissions.map((sub) => (
                  <div key={sub.platform} className="flex items-center gap-1.5 text-xs text-slate-400">
        ...
        {canSubmit && (
          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6 mb-6">
            ...
            <p className="text-slate-400 text-sm mb-4">
              Publica el clip en las plataformas y pega las URLs aquí. Mínimo 1 URL obligatoria, las demás son opcionales. Las vistas de todas las plataformas se suman.
            </p>
                  ...
                  <div key={platform}>
                    ...
                    <input
                      ...
                      className={`w-full bg-slate-900 border text-white rounded-lg px-4 py-3 text-sm placeholder-slate-500 focus:outline-none transition-colors ${
                        err ? 'border-red-500 focus:border-red-400' : 'border-slate-600 focus:border-purple-500'
                      }`}
                    />
        ...
        {claim.status === 'SUBMITTED' && submissions.length === 0 && claim.socialUrl && (
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-5 mb-6">
            ...
            <a href={claim.socialUrl} target="_blank" rel="noopener noreferrer" className="text-purple-400 hover:text-purple-300 text-sm break-all">
              {claim.socialUrl}
            </a>
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-slate-400">{new Date(claim.createdAt).toLocaleString('es-ES')}</div>
              </div>
            </div>
            {claim.submittedAt && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-400">{new Date(claim.submittedAt).toLocaleString('es-ES')}</div>
                  {submissions.length > 0 && (
                    <div className="flex gap-2 mt-1">
                      {submissions.map((s) => (
                        <span key={s.platform} className="text-xs text-slate-500">{PLATFORM_CONFIG[s.platform].icon} {PLATFORM_CONFIG[s.platform].label}</span>
                      ))}
                    </div>
                  )}
                </div>
            ...
            {claim.verifiedAt && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-400">{new Date(claim.verifiedAt).toLocaleString('es-ES')}</div>
                </div>
              </div>
            )}
          </div>

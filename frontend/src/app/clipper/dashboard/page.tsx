    ...
    <div className="min-h-screen">
      <header className="border-b border-slate-800 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 text-sm">
          <Link href="/marketplace" className="text-slate-400 hover:text-white transition-colors">Marketplace</Link>
          <Link href="/clipper/claims" className="text-slate-400 hover:text-white transition-colors">Mis Claims</Link>
          <Link href="/clipper/earnings" className="text-slate-400 hover:text-white transition-colors">Ganancias</Link>
        </div>
      </header>
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
              <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
                ...
                <div className="text-3xl font-bold text-green-400 mt-2">${data.totalEarnings.toFixed(2)}</div>
              </div>
              <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
              ...
              <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
                ...
                <div className="text-3xl font-bold text-yellow-400 mt-2">{data.pendingSubmissions}</div>
              </div>
            </div>

            {/* Quick Actions */}
            <div className="flex flex-wrap gap-3 mb-8">
              <Link href="/marketplace" className="bg-purple-600 hover:bg-purple-700 text-white font-medium px-4 py-2 rounded-lg transition-colors text-sm">
                Buscar clips
              </Link>
              <Link href="/clipper/claims" className="bg-slate-700 hover:bg-slate-600 text-white font-medium px-4 py-2 rounded-lg transition-colors text-sm">
                Ver todos mis claims
              </Link>
              <Link href="/clipper/earnings" className="bg-slate-700 hover:bg-slate-600 text-white font-medium px-4 py-2 rounded-lg transition-colors text-sm">
            ...
            <div>
              ...
              {data.recentClaims.length === 0 ? (
                <div className="text-center py-12 bg-slate-800/50 border border-slate-700 rounded-xl">
                  ...
                  <Link href="/marketplace" className="text-purple-400 hover:text-purple-300 text-sm mt-2 inline-block">
                  ...
                  {data.recentClaims.map((claim) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 text-sm">
                        {claim.earnings > 0 && (
                          <span className="text-green-400 font-medium">${claim.earnings.toFixed(2)}</span>
                        )}
                        <span className={`font-medium ${STATUS_COLORS[claim.status] || 'text-slate-400'}`}>

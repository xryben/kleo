...
interface SocialStatus { connected: boolean; username?: string; channelName?: string; tokenExpires?: string; }

const PLATFORMS = [
  {
    key: 'ig', label: 'Instagram', emoji: '📸',
  ...
  {
    key: 'yt', label: 'YouTube Shorts', emoji: '▶️',
...
function SettingsContent() {
  ...
  useEffect(() => {
    ...
    for (const [param, msg] of Object.entries(toasts)) {
      ...
      if (params.get(k) === v) { setToast(msg); break; }
    }

    Promise.all(PLATFORMS.map((p) => p.api.status().then((s: SocialStatus) => [p.key, s] as [string, SocialStatus])))
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-2xl mx-auto px-6 py-8 space-y-6">
        {toast && (
          <div className={`px-4 py-3 rounded-xl text-sm ${toast.startsWith('✅') ? 'bg-green-500/10 border border-green-500/30 text-green-400' : 'bg-red-500/10 border border-red-500/30 text-red-400'}`}>
            {toast}
          </div>
        )}
        ...
        {/* Social platforms */}
        <div>
          <h2 className="text-sm font-medium text-slate-400 uppercase tracking-wide mb-3">Redes sociales</h2>
                ...
                <div key={p.key} className="bg-slate-800/50 border border-slate-700 rounded-xl p-5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`w-9 h-9 bg-gradient-to-br ${p.gradient} rounded-lg flex items-center justify-center text-lg`}>
                        {p.emoji}
                      </div>
                      <div>
                        ...
                        {loading ? (
                          ...
                          <div className="text-xs text-green-400">{p.prefix}{name as string}</div>
                        ) : (
                    ...
                    {!loading && (
                      status?.connected ? (
                        <button onClick={() => handleDisconnect(p.key, p.api)} className="text-xs text-red-400 hover:text-red-300">
                          Desconectar
                        </button>
                      ) : (
                        <button
